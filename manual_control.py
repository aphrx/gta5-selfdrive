...
def direction(x, y):
    ...
    #     last_x = x
    # else:
    #     if(last_x > x) and (last_x is not None) and (last_dir is not None):
    #         last_dir = last_dir * -1
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        temp = '%s' % ' and '.join(text)
        print(temp)
        global steer
        if ("Solid" in temp) or ("Broken" in temp) or ("SolidSolid" in temp) or ("SolidBroken" in temp) or ("BrokenSolid" in temp):
            print(temp)
            # Amal's code
...
class CameraManager(object):
    ...
    def region_of_interest(img):
        height = img.shape[0]
        width = img.shape[1]
        interest = np.array([[(0, height),(0,(height/3)*2), (width/4, height/2), ((width/4)*3, height/2), (width, (height/3)*2), (width, height)]])
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            processed_img = CameraManager.process_img(CameraManager.process_img(array2))
            ...
            lined_img, m1, m2 = CameraManager.display_lines2(processed_img, lines)

            cv2.imshow('window', lined_img)
            cv2.waitKey(1)
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # Amal's code
            world.player.apply_control(carla.VehicleControl(throttle=0.5, steer=steer))
            direction(world.player.get_location().x, world.player.get_location().y )

            # print(world.player.get_location().x)
            # print(world.player.get_location().y)
            pygame.display.flip()
